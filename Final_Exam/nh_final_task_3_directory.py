...
class SampleGui(GuiBaseClass, GenericClasse):
    def __init__(self, root, dir_name: str):
        ...
        self.combobox_values = ["GO ID", "KEGG", "DOI"]
        ...
        self.text2 = tk.Text(self.bottom_frame,width=20)
        DGScrolled(self.text2)
        ...
        self.bottom_frame.add(self.text2)
    ...
    def button_on_click(self):
        ...
        self.text.insert(tk.END, combobox_text + "\n")
    ...
    def list_on_click(self, event):
        ...
        value = w.get(index)
        print(f"{self.absolute_path_mapping[value]}")
        print(f"{os.path.isfile(self.absolute_path_mapping[value])}")
    ...
    def open_dir_name(self):
        ...
        names = os.listdir(self.dir_name)
        file_names = [name for name in names if name.endswith(".dat") or name.endswith(".dat.gz")]
        self.list_box.delete(0, tk.END)
        for file_name in file_names:
            self.list_box.insert(tk.END, file_name)
            self.absolute_path_mapping.update({file_name: self.dir_name + "/" + file_name for file_name in file_names})
...
if __name__ == "__main__":
    ...
    if len(args) == 1 or "-h" in args or "--help" in args:
        ...
        if "-gui" in args:
            ...
            sample_gui: SampleGui = SampleGui(root=root, dir_name=file_or_dir_name)
